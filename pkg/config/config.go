...
type RegistryConfig struct {
	...
	// TLS: cert/key files for HTTPS (generated by mkube-installer)
	TLSCertFile string `yaml:"tlsCertFile"` // path to PEM cert (default /etc/registry/tls.crt)
	TLSKeyFile  string `yaml:"tlsKeyFile"`  // path to PEM key (default /etc/registry/tls.key)
	// CA certificate file for clients to verify the registry
	TLSCACertFile string `yaml:"tlsCACertFile"` // path to CA PEM (clients load this)
